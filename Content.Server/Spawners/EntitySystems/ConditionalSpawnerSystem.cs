...
public sealed partial class ConditionalSpawnerSystem : EntitySystem
{
    ...
    private void Spawn(EntityUid uid, ConditionalSpawnerComponent component)
    {
        ...
        var entity = Spawn(toSpawn, coords, rotation: rotation);
        RaiseRandomItemSpawned(entity); // Prospect
    }
    ...
    private void Spawn(EntityUid uid, RandomSpawnerComponent component)
    {
        ...
        // Prospect: spawn the prototype GetPrototype actually chose (upstream
        // discards it and re-picks from Prototypes, which loses rare spawns),
        // carry scatter over to the spawn, and jack it to events
        var entity = Spawn(proto, coordinates, rotation: rotation);
        if (TryComp<PSScatterOnSpawnComponent>(uid, out var scatter))
            AddComp(entity, new PSScatterOnSpawnComponent { Force = scatter.Force });
        RaiseRandomItemSpawned(entity);
        // End Prospect
    }
        ...
        Angle rotation)
    {
        ...
        foreach (var (protoId, count) in prototypeStacks.Values)
        {
            ...
            // Prospect: jack stacked spawns to events
            foreach (var stacked in _stack.SpawnMultipleAtPosition(protoId, count, entCoordinates))
            {
                RaiseRandomItemSpawned(stacked);
            }
            // End Prospect
        }
    }
    ...
    private EntityUid SpawnAtRandomOffset(EntProtoId proto, MapCoordinates coords, float offset, Angle rotation)
    {
        ...
        var entity = Spawn(proto, trueCoords, rotation: rotation);
        RaiseRandomItemSpawned(entity); // Prospect
        return entity;
    }
    ...
    /// <summary>
    /// Prospect: announce a spawner-produced entity so ProspectableSystem can
    /// attach level-based stat components to it.
    /// </summary>
    private void RaiseRandomItemSpawned(EntityUid entity)
    {
        var ev = new RandomItemSpawnedEvent(entity);
